...
//扫描目录
void DirectionList(const string &path, vector<string> &subfile, vector<string> &subdir)//文件和目录分别存储
{
	string _path = path;
	_path += "\\*.*";
	...
	_finddata_t file;
	//句柄
	long handle = _findfirst(_path.c_str(), &file); //遍历第一个文件
	if(handle == -1)
	{
		cout<<"扫描目录出错......"<<endl;
		return;
	...
	do
	{
		if(file.name[0] == '.')
			continue;
		//是目录将目录名存入
		if(file.attrib &  _A_SUBDIR)  //判断文件属性是否为文件夹
			subdir.push_back(file.name);
		//是文件将文件名存入
		else
			subfile.push_back(file.name);
		
	}while(_findnext(handle, &file) == 0);  //遍历下一个文件
